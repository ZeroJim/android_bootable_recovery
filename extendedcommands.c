...
{
    static char* headers[] = {  "选择ZIP包刷入",
                                "",
                                NULL
    };
    
    char* install_menu_items[] = {  "从SD卡中选择",
                                    "通过ADB sideload刷入",
                                    "刷入SD卡update.zip包",
                                    "开启/关闭签名验证",
                                    NULL,
    ...
    if (volume_for_path("/emmc") != NULL) {
        other_sd = "/emmc/";
        install_menu_items[4] = "从内置SD卡选择";
    }
    else if (volume_for_path("/external_sd") != NULL) {
        other_sd = "/external_sd/";
        install_menu_items[4] = "从外置SD卡选择";
    }
...
int confirm_selection(const char* title, const char* confirm)
{
    ...
    char* confirm_headers[]  = {  title, "  操作不可恢复，请慎重！.", "", NULL };
...
void show_nandroid_advanced_restore_menu(const char* path)
{
    ...
    static char* advancedheaders[] = {  "选择要恢复的镜像",
                                "",
                                "先选择一个镜像",
                                "然后你会看到",
                                "更多选项.",
    ...
    {
        case 0:
            if (confirm_selection(confirm_restore, "好的 - 恢复 boot"))
                nandroid_restore(file, 1, 0, 0, 0, 0, 0);
            break;
        case 1:
            if (confirm_selection(confirm_restore, "好的 - 恢复 system"))
                nandroid_restore(file, 0, 1, 0, 0, 0, 0);
            break;
        case 2:
            if (confirm_selection(confirm_restore, "好的 - 恢复 data"))
                nandroid_restore(file, 0, 0, 1, 0, 0, 0);
            break;
        case 3:
            if (confirm_selection(confirm_restore, "好的 - 恢复 cache"))
                nandroid_restore(file, 0, 0, 0, 1, 0, 0);
            break;
        case 4:
            if (confirm_selection(confirm_restore, "好的 - 恢复 sd-ext"))
                nandroid_restore(file, 0, 0, 0, 0, 1, 0);
            break;
        case 5:
            if (confirm_selection(confirm_restore, "好的 - 恢复 wimax"))
    ...
    for (;;)
    {
        ...
        switch (chosen_item)
        {
            ...
            case 3:
                ...
                if (confirm_selection( "确定清除?", "好的 - 清除缓存")) {
...
void handle_failure(int ret)
{
    ...
    ui_print("/tmp/recovery.log 已经被复制到 /sdcard/clockworkmod/recovery.log. 请sina提交给@zerojim-俊熙\n");
}
...
int verify_root_and_recovery() {
    ...
    if (!exists) {
        ui_show_text(1);
        ret = 1;
        if (confirm_selection("Root权限丢失. 是否Root您的设备?", "是的 - 帮我ROOT (/system/xbin/su)")) {
